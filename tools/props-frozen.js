/* props-frozen.js — the levitating props contract, measured with the levitation
   switched off.

   tools/props.js is that contract, and it is measured through the browser's own
   rects, so it includes the CSS motion: `.lev-bob` slides its prop up and down
   under `7s steps(7,end) infinite alternate`, which means each prop HOLDS a given
   offset for a whole second before jumping to the next one. Where a prop sits
   when a sample is taken is therefore a fact about when the page booted, not
   about where the prop is drawn -- and `blankStepsWorst` counts the scroll
   samples before the lowest prop crosses the bottom of the sky, which is exactly
   the edge a one-second dwell of three art units can move by one sample.

   This is that same probe with the four levitation animations turned off, and
   that is the only change: same fourteen shapes, same ladder, same lanes, same
   eighteen checks. Off, a prop sits where it is authored, every sample of every
   run agrees, and the reading stops depending on the clock.

   It exists to settle the one question tools/props.js cannot settle by itself:
   when `blankStepsWorst` disagrees between two pages, is the drawing different,
   or was the clock? Identical readings here alongside a one-sample disagreement
   there means the clock.

   Generated from tools/props.js by inserting the stylesheet below. The selectors
   are the same four the reduced-motion block in app/globals.css already turns
   off, so this is a state the page can genuinely be found in. */
(async () => {
  ...
  /* Freeze the levitation before a single rect is read. Same four selectors the
     reduced-motion block already turns off, and nothing else -- the ladder, the
     lanes and the boxes below are untouched, so a difference that survives this
     is a difference in the drawing and not in the clock. */
  const freeze = document.createElement('style');
  freeze.textContent = '.lev-bob,.lev-sway,.lev-flip,.lev-spin{animation:none!important}';
  ...
  return {
    ...
    frozen: true, blankStepsWorst: blankWorst, blankAtFrac: blankAt, seenAtEnd,
